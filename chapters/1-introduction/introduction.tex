...
For example, the regex \texttt{b(?=a)} will the search for the letter $b$
...
for the last character of the pattern. On the other hand, if try to match \texttt{cb},
...
loops. This paper also states that that algorithm can easily be adapted to
both ECMAScript and Rust Semantics. During this project, we emitted doubts
on the existence of the second adaptation (at least for the stated time and
...
known as threads and tries to find how all these states evolve :
\begin{itemize}
    \item First, it tries to apply all possible free moves.
    \item Then it tries to consume the character for all the threads.
\end{itemize}
