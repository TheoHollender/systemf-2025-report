...
then there would be two parts that we would be interested in optimizing :
\begin{itemize}
...
on distinct cells, there is no optimization possible, so we will store the two modifications
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item When a combination happens between two complete forms, it takes $O(K)$ times and decrements
